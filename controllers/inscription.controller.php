...
function handle_request() {
    ...
    if(!isset($_POST['nom'], $_POST['prenom'], $_POST['email'], $_POST['date_naissance'], $_POST['pseudo'], $_POST['password'])){
        ...
        //utilisation de la fonction password_hash pour hasher le mot de passe
        $password_h = password_hash($password, PASSWORD_DEFAULT);

        //appel de la fonction set_data pour insérer les données dans la base de données
        $response = set_data($nom, $prenom, $email, $date_naissance, $pseudo, $password_h);
        if($response){
            // echo $response['message'];

            //redirection vers la page de connexion si l'inscription est effectuée avec succès
